...
def show_all_contact(contacts: dict):
    ...
    for name, phone in contacts.items():
        result += "|\t" + f"{name}" + str("\t" if len(name) < 8 else "") + "\t|\t" + f"{phone}" + str("\t" if len(phone) < 8 else "") + "\t\t|\n"
        result += "-" * l + "\n"
...
def write_db(db_name, contacts: dict):
    ...
    if db_is_exist(db_name):
        answer = input(f"{Fore.RED}The database exists. Do you want to rewrite all the data? (y/N):{Style.RESET_ALL} ").lower().strip()
        if answer != 'y':
...
def main():
    ...
    while True:
        user_input = input(f"{Fore.WHITE}Enter a command or 'h' for help:{Style.RESET_ALL} ") 
        if user_input:
        ...
        if command in ["close", "exit", "e"]:
            ...
            answer = input(f"{Fore.RED}Are you sure to clear all data? (y/N):{Style.RESET_ALL} ").lower().strip()
            if answer == "y":
